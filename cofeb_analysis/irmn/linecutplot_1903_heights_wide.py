...
for j in range(0, numheights):
    nleftfms[j] = np.append(nleftfms[j],glob.glob(simpath+'nl_*'+str(heights[j])+'*lowres_1903.txt'))
    nleft[j] = [[],[],[]]
    for k in range(0, 3):
        nleft[j][k] = np.loadtxt(nleftfms[j][k], delimiter=',')

#simulation constants
ssize = 2.5
slen = len(nleft[0][0][0])
sres = ssize/slen

nleftnv = np.zeros((numheights, slen, slen))

for m in range(0, numheights):
    for j in range(0, slen):
        for i in range(0, slen):
            nleftnv[m][j,i] = np.multiply(sfieldpre,np.abs((nleft[m][0][j,i]*np.sin(theta)*np.cos(phi))+
                                                         (nleft[m][1][j,i]*np.sin(theta)*np.sin(phi))+
                                                         (np.add(nleft[m][2][j,i],zfields)*np.cos(theta))))
...
# for i in range(0,numheights):
#     plt.plot(nlcut[i][0],nlcut[i][1],linewidth=2.0, label=str(heights[i]))
# plt.plot(nlcut[dw][1][0],nlcut[dw][1][1],color='#F97304',linewidth=2.0, label=u'left-handed Néel')
plt.plot(nlcut[2][0],nlcut[2][1],linewidth=2.0, color='#2D7DD2', label=str(heights[2]))
