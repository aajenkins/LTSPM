...
for i in range(0, len(errnames)):
    ...
    nright[i] = [[],[],[]]
    for k in range(0, 3):
        bloch[i][k] = fieldpre*np.loadtxt(blochfms[i][k], delimiter=',')
    bloch[i][2] = bloch[i][2]+zfields
    for k in range(0, 3):
        nleft[i][k] = fieldpre*np.loadtxt(nleftfms[i][k], delimiter=',')
    nleft[i][2] = nleft[i][2]+zfields
    for k in range(0, 3):
        nright[i][k] = fieldpre*np.loadtxt(nrightfms[i][k], delimiter=',')
