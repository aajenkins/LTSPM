...
* @Last modified time: 2017-02-13T21:24:22-08:00
*/
...
double Fx(double mz[DLEN][DLEN], double phi[DLEN][DLEN]);
double Fy(double mz[DLEN][DLEN], double phi[DLEN][DLEN];
double alpha_z(double x, double y);
double alpha_xy(double x, double y);
...
double get_Fx(double mz[DLEN][DLEN], double phi[DLEN][DLEN]) {
  double Fx = 0;
  dxmz = double dxmz = (mz[y][x+1]-mz[y][x-1])/2;
  alpha_z()
}
